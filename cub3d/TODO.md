- ~~Se o teu ft_error chama ft_free_game, vais ter o mesmo crash (Segfault) se o raycasting falhar, porque estás a limpar memória dentro do loop.~~ **Resolved by Story 6.1**: `error_flag` + `mlx_loop_end` pattern. `ft_draw_loop` sets the flag instead of calling `ft_error`; cleanup happens in `main` after `mlx_loop` returns.
